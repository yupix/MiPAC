...
class AdminAnnouncementActions(AbstractAction):
    def __init__(
        self,
    ...
    async def create(
        self, title: str, text: str, image_url: str | None = None
    ) -> Announcement:
    ...
    async def gets(
        ...
        async def request(req_body) -> list[AnnouncementSystem]:
            res: list[IAnnouncementSystem] = await self.__session.request(
                Route('POST', '/api/admin/announcements/list'),
                auth=True,
                json=req_body,
            )
            return [
                AnnouncementSystem(announcement, client=self.__client)
